...
class ConvProcessingBlockBNRC(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvProcessingBlockBNRC, self).__init__()
...
class ConvProcessingResBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvProcessingResBlock, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict['conv_2'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['batch_norm_2'] = nn.BatchNorm2d(self.num_filters)

        out = self.layer_dict['conv_2'].forward(out)
        out = self.layer_dict['batch_norm_2'].forward(out)
...
class ConvDimReductionResBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ConvDimReductionResBlock, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict['conv_shortcut'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                                kernel_size=(1,1), padding=0, stride = 2)

        self.layer_dict['batch_norm_shortcut'] = nn.BatchNorm2d(self.num_filters)

        out = self.layer_dict['conv_2'].forward(out)
        out = self.layer_dict['batch_norm_2'].forward(out)

        shortcut = self.layer_dict['conv_shortcut'].forward(shortcut)
        shortcut = self.layer_dict['batch_norm_shortcut'].forward(shortcut)
